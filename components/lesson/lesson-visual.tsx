  ...
  return (
    <div className="lesson-visual" data-template={template} data-reduced-motion={reducedMotion} role="img" aria-label={`${segment.narration} ${segment.learnerShouldNotice.join("。")}`}>
      <svg viewBox="0 0 960 540" aria-hidden="true">
        ...
        {["derivative_secant_to_tangent", "derivative_same_value_different_slope"].includes(template) ? <SecantVisual model={mathModel} counterexample={template.includes("same_value")} /> : null}
        {template === "derivative_limit_definition" ? <LimitVisual /> : null}
        {["derivative_worked_example", "derivative_algebra_expansion_repair", "derivative_cancel_h_repair"].includes(template) ? <AlgebraVisual model={mathModel} repair={template} /> : null}
        {template === "derivative_function_derivative_link" ? <DerivativeLink model={mathModel} /> : null}
      </svg>
...
function SecantVisual({ model, counterexample }: { model: Model; counterexample: boolean }) {
...
function AlgebraVisual({ model, repair }: { model: Model; repair: string }) {
...
function DerivativeLink({ model }: { model: Model }) {
...
function evaluate(model: Model, x: number) { return model.coefficients.reduce((sum, coefficient, degree) => sum + coefficient * x ** degree, 0); }
function derivativeAt(model: Model, x: number) { return model.coefficients.slice(1).reduce((sum, coefficient, index) => sum + coefficient * (index + 1) * x ** index, 0); }

function makePlot(model: Model, includeDerivative: boolean) {
