...
export function LessonPlayer({ lesson, initialStoryState, initialLearnerState }: { lesson: LessonSpec; initialStoryState: LessonStoryState; initialLearnerState: LessonLearnerState }) {
  ...
  const step = lesson.guidedPractice.steps[practiceStep];
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try {
      const value = JSON.parse(saved) as { storyState: LessonStoryState; learnerState: LessonLearnerState; phase: Phase; practiceStep: number };
  ...
  useEffect(() => { window.localStorage.setItem(storageKey, JSON.stringify({ storyState, learnerState, phase, practiceStep })); }, [learnerState, phase, practiceStep, storageKey, storyState]);
  ...
  function commitCheckpoint() {
    ...
    if (!correct) {
      const template = checkpoint.id === "checkpoint-secant"
        ? "derivative_limit_definition"
        : "derivative_same_value_different_slope";
  ...
  async function submitStep() {
    if (!step || !expression.trim() || busy) return;
    setBusy(true); setFeedback("");
    try {
      const response = await fetch(`/api/lessons/${lesson.id}/attempts`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ stepId: step.id, expression, learnerState }) });
      ...
      if (data.correct) {
        window.setTimeout(() => { setExpression(""); setFeedback(""); if (practiceStep === lesson.guidedPractice.steps.length - 1) { setStoryState((current) => ({ ...current, taskStatus: "transfer" })); setPhase("transfer"); } else setPracticeStep((value) => value + 1); }, 650);
  ...
  function insertSymbol(symbol: string) { setExpression((value) => value + symbol); inputRef.current?.focus(); }
  ...
  return (
    <main className="lesson-shell" data-phase={phase}>
      <header className="lesson-header"><Link href="/" className="lesson-brand">PLOT AS PROOF</Link><div><span>CALCULUS LAB · DERIVATIVE</span><strong>{lesson.objective}</strong></div><p>{Math.min(100, Math.round(((storyState.currentSegmentIndex + (phase === "practice" ? 2 : phase === "transfer" ? 3 : 0)) / (lesson.segments.length + 3)) * 100))}%</p></header>
      <section className="lesson-workspace">
        ...
        <div className="lesson-main">
          ...
          {phase === "practice" && step ? <section className="practice-card"><p>GUIDED PRACTICE · {practiceStep + 1}/4</p><h2>{lesson.guidedPractice.prompt}</h2><div className="practice-progress">{lesson.guidedPractice.steps.map((item, index) => <span key={item.id} className={index < practiceStep ? "complete" : index === practiceStep ? "active" : ""}>{index + 1}</span>)}</div><label htmlFor="math-expression">{step.prompt}</label><input ref={inputRef} id="math-expression" value={expression} onChange={(event) => { setExpression(event.target.value); setFeedback(""); }} placeholder={step.placeholder} autoComplete="off"/><div className="math-preview"><span>PREVIEW</span><strong>{expression || "Your expression will appear here"}</strong></div><div className="math-keypad">{["x", "h", "^", "(", ")", "+", "-", "*", "/"].map((symbol) => <button key={symbol} onClick={() => insertSymbol(symbol)}>{symbol}</button>)}<button onClick={() => setExpression("")}>Clear</button></div>{feedback ? <p className="practice-feedback" role="status">{feedback}</p> : null}<button className="practice-submit" disabled={!expression.trim() || busy} onClick={submitStep}>{busy ? "Checking…" : "Check this step"}</button></section> : null}
          {phase === "remediation" ? <section className="remediation-card"><p>ADAPTIVE REPRESENTATION</p><LessonVisual segment={remediationSegment} mathModel={storyState.taskStatus === "practice" ? lesson.guidedPractice.function : lesson.mathModel} reducedMotion={reducedMotion}/><h2>{remediationSegment.narration}</h2><button onClick={() => { setRemediationTemplate(null); setPhase(storyState.taskStatus === "practice" ? "practice" : "lesson"); if (storyState.taskStatus !== "practice") advanceSegment(); }}>I see the difference — continue</button></section> : null}
...
function LessonRecap({ lesson, learnerState, transferResult, onReplay }: { lesson: LessonSpec; learnerState: LessonLearnerState; transferResult: { correct: boolean; normalizedAnswer: string }; onReplay: () => void }) {
  ...
  return <main className="lesson-recap"><p>LEARNING EVIDENCE</p><h1>{transferResult.correct ? "You transferred the relationship to a new function." : "The transfer exposed a connection that still needs practice."}</h1><p>The current evidence shows {learnerState.completedStepIds.length}/4 guided steps completed, {correctCheckpoints}/2 diagnostic answers correct, and an unassisted transfer result that was {transferResult.correct ? "correct" : "not yet correct"}. This is evidence, not a final mastery claim.</p><div><article><span>TRANSFER</span><strong>{transferResult.normalizedAnswer}</strong><small>{transferResult.correct ? "Independent result correct" : "Revisit difference-quotient simplification"}</small></article><article><span>POSSIBLE MISCONCEPTIONS</span><strong>{learnerState.possibleMisconceptions.length}</strong><small>{learnerState.possibleMisconceptions.join(" · ") || "No specific misconception recorded"}</small></article></div><button onClick={onReplay}>Replay with another path</button><Link href="/create">Try another derivative question</Link><small>{lesson.objective}</small></main>;
}
