...
## 2026-07-21 — Teach any STEM topic with a two-track whiteboard pipeline

Input outside the one-variable derivative grammar (and any PDF upload) now produces a narrated animated lesson instead of `UNSUPPORTED_CALCULUS_SCOPE`. A new `LessonSpec` v3 joins the schema union additively. GPT-5 authors a lesson as a constrained JSON "whiteboard scene DSL" (`lib/lesson/whiteboard-dsl.ts`): typed elements (text, formula, axes, plot, point, mathline, shapes, label) and a timeline of actions anchored to exact narration phrases. Track A renders that DSL in a fixed browser player (`components/lesson/whiteboard-stage.tsx`, SVG + GSAP + KaTeX) with the timeline slaved to the narration audio clock, so it plays within seconds and never executes model code. Every scene is validated against the supported plot grammar and KaTeX, then sanitized (synthesize a missing axes, drop dangling references, downgrade unmatched anchors) so a stray model mistake degrades gracefully instead of failing the lesson; only genuinely unfixable scenes trigger a bounded regeneration. Correctness is gated by deterministic numeric spot-checks of the lesson's machine-expressible claims (hard gate, forces one regeneration) plus an advisory second-model fact-check (recorded, non-blocking). Checkpoints for generic topics are inline multiple-choice questions graded in the player. The existing derivative template pipeline and seeded demo are unchanged and still handle clear derivative questions.

## 2026-07-21 — Track B upgrades whiteboard segments to cinematic Manim in the background

Track B always attempts, per segment, to replace the whiteboard visual with a 3Blue1Brown-style Manim render. GPT-5 writes a Manim CE 0.20.1 `GeneratedScene` within a constrained skeleton; the renderer's new `POST /v1/renders/custom` first runs `renderer/validate.py` (a Python `ast` allowlist — only manim/math/numpy imports, no dunder access, no `open`/`eval`/`exec`/OS/network) and only then executes the scene inside the existing hardened container (read-only FS, tmpfs work dir, cap-drop, pids/mem/cpu limits, localhost bind, `--network none`). Render failures return the traceback tail so codegen can self-repair, up to three attempts per segment under a wall-clock budget. A cheap liveness probe runs before any code generation so a configured-but-dead renderer never burns model calls. Completed segments are hot-swapped into the stored lesson (atomic tmp+rename write) and the player polls and swaps them in at a pause or section boundary. If every attempt fails, the whiteboard version remains — an acceptable end state, not an error. This deliberately supersedes the former "never execute model-authored Python" invariant, now narrowed to "only inside the validated sandbox."

## 2026-07-21 — Narration timestamps drive word-level caption and animation sync
